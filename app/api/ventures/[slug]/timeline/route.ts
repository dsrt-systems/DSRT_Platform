...
export async function GET(req: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!venture) return NextResponse.json({ timeline: [] })
  const { data } = await supabase.from('venture_timeline').select('*').eq('venture_id', venture.id).eq('is_public', true).order('event_date', { ascending: false })
...
export async function POST(req: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  const { data, error } = await supabase.from('venture_timeline').insert({ ...body, venture_id: venture.id, created_by: user.id }).select().single()
