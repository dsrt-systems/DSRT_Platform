...
export async function POST(req: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  if (!body.alternative_name) {
    return NextResponse.json({ error: 'alternative_name required' }, { status: 400 })
  }

  const { count } = await supabase
    .from('venture_alternatives')
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      alternative_name: String(body.alternative_name).trim().slice(0, 200),
      what_works: body.what_works || null,
      where_fails: body.where_fails || null,
      position: count || 0,
    })
    .select()
    .single()
