...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data: opportunity } = await supabase.from('opportunities')
      .select('id, title, slug, poster_user_id, status, applications_open, application_deadline, max_applications, application_count, project_id, venture_id')
      .eq('id', id)
      .single()
    ...
    if (opportunity.status !== 'active' && opportunity.status !== 'closing-soon') {
      return NextResponse.json({ error: 'Opportunity is not accepting applications' }, { status: 400 })
    }
    ...
    if (opportunity.max_applications && opportunity.application_count >= opportunity.max_applications) {
      return NextResponse.json({ error: 'Maximum applications reached' }, { status: 400 })
    }

    // Duplicate check
    const { data: existing } = await supabase.from('opportunity_applications')
      .select('id').eq('opportunity_id', id).eq('applicant_id', user.id).maybeSingle()
    if (existing) {
    ...
    const { data: applicantProfile } = await supabase
      .from('users')
      .select('full_name, username, avatar_url, tagline, bio, location, is_verified')
      .eq('id', user.id)
      .single()
    ...
    // Insert application
    const { data: application, error } = await supabase.from('opportunity_applications').insert({
      opportunity_id: id,
    ...
    const applicantName = applicantProfile?.full_name || applicantProfile?.username || 'A builder'
    ...
    if (opportunity.project_id) {
      ...
      const { data: proj } = await supabase.from('projects').select('name, slug').eq('id', opportunity.project_id).single()
      ...
      const { data: vent } = await supabase.from('ventures').select('name, slug').eq('id', opportunity.venture_id).single()
    ...
    }).then(() => {}, (e) => console.error('Inbox insert failed:', e))
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { error } = await supabase.from('opportunity_applications')
      .update({ pipeline_stage: 'withdrawn', status: 'withdrawn' })
      .eq('opportunity_id', id)
      .eq('applicant_id', user.id)
