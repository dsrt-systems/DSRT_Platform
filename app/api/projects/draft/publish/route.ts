...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { id, visibility, show_in_explore, domains, primary_domain, technologies, looking_for_roles } = body

    if (!id) return NextResponse.json({ error: 'Project ID is required' }, { status: 400 })

    const isPublic = visibility === 'public'
    
    // Generate canonical project_number (PRJ-XXXXXX)
    ...
    const { data: project, error: updateError } = await supabase
      ...
      .select('id, name, slug, industry, location, tagline, project_number')
      .single()
    ...
    if (domains && domains.length > 0) {
      ...
      if (domainRows && domainRows.length > 0) {
        const domainAssignments = domainRows.map((d: any) => ({
          project_id: id,
          domain_id: d.id,
          is_primary: d.name === primary_domain
        }))
    ...
    // Global Activity Signal (wrapped safely without .catch chaining)
    if (isPublic) {
      try {
