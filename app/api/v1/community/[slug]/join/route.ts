...
export async function POST(
  ...
  try {
    ...
    const result = await joinCommunity(supabase, ctx.identityId, communityId, ctx.requestId)

    return ok(
      { membership_id: result.membership_id, status: result.status },
      { ctx, eventId: result.event_id }
    )
  } catch (err) {
    return fail(err, ctx)
  }
