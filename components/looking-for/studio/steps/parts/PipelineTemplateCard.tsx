...
import { Plus, ArrowUp, ArrowDown, LockSimple, Trash } from '@phosphor-icons/react'
...
export function PipelineTemplateCard() {
  ...
  const load = useCallback(async () => {
    const res = await fetch(`/api/opportunities/${oppId}/stages`)
    const d = await res.json()
    if (res.ok) setStages(d.stages || [])
    else setStages([])
  }, [oppId])
  ...
  const rename = async (id: string, name: string) => {
    setBusy(id)
    try {
      await fetch(`/api/opportunities/${oppId}/stages`, {
        method: 'PATCH',
  ...
  const remove = async (s: any) => {
    ...
    try {
      const res = await fetch(
        `/api/opportunities/${oppId}/stages?stage_id=${s.id}`,
        { method: 'DELETE' }
      )
      if (!res.ok) {
  ...
  return (
    <div className="rounded-2xl border border-zinc-800/80 bg-gradient-to-b from-[#18181b] via-[#121215] to-[#0f0f11] overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/80">
        <div>
          ...
          <p className="text-[11.5px] text-zinc-500 mt-0.5">
            Rename core stages, reorder, or add custom stages. Core stages cannot
            be deleted.
          </p>
        </div>
        <button
      ...
      {adding && (
        <div className="p-4 border-b border-zinc-800/70 bg-zinc-950/30 grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-2">
          <input
            value={newName}
          ...
          <input
            ...
            onChange={(e) =>
              setNewKey(
                e.target.value.replace(/[^a-z0-9\-]/g, '-').toLowerCase()
              )
            }
      ...
      {stages === null ? (
        ...
        <div className="p-6 text-[12.5px] text-zinc-500">
          Pipeline template not initialized yet. Add your first stage above.
        </div>
      ) : (
