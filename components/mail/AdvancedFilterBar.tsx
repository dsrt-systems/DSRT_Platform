...
import { 
  ...
  User, Buildings, Rocket, CalendarBlank, CaretDown, Check
...
const FROM_OPTIONS = [
  ...
  { value: 'user', label: 'People', icon: User, color: 'text-blue-400' },
  { value: 'project', label: 'Projects', icon: Rocket, color: 'text-emerald-400' },
  { value: 'venture', label: 'Ventures', icon: Buildings, color: 'text-violet-400' },
]
...
export function AdvancedFilterBar({ filters, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    const close = () => { setFromOpen(false); setDateOpen(false) }
  ...
  const chipBase = "flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[11.5px] font-semibold transition-colors border whitespace-nowrap"
  const chipInactive = "bg-transparent border-white/[0.06] text-white/55 hover:text-white hover:bg-white/[0.03] hover:border-white/[0.1]"
  const chipActive = "bg-white/[0.08] border-white/[0.15] text-white"

  const dropdownClass = cn(
    "fixed z-[9999] w-[200px] rounded-lg overflow-hidden",
    "bg-[#141419] border border-white/[0.12]",
    "shadow-[0_16px_48px_rgba(0,0,0,0.8)]"
  )

  return (
    <div className="relative z-20 border-b border-white/[0.06] bg-[#0a0a0f] flex-shrink-0">
      <div className="flex items-center gap-1.5 px-3 h-10 overflow-x-auto scrollbar-hide">
        {/* Filter count indicator */}
        <div className={cn(
          "flex items-center gap-1.5 h-7 px-2 rounded-md border",
          activeCount > 0
            ? "bg-white/[0.06] border-white/[0.12] text-white"
            : "border-white/[0.04] text-white/40"
        )}>
          <Funnel className="w-3 h-3" weight={activeCount > 0 ? "fill" : "regular"} />
          {activeCount > 0 && (
        ...
        >
          <EnvelopeOpen className="w-3 h-3" />
          Unread
        </button>
        ...
        >
          <Star className="w-3 h-3" weight={filters.isStarred ? "fill" : "regular"} />
          Starred
        </button>
        ...
        >
          {selectedFrom?.icon && (
            <selectedFrom.icon className={cn("w-3 h-3", selectedFrom.color)} weight="fill" />
          )}
        ...
        {activeCount > 0 && (
          <button
            onClick={clearAll}
            className="flex items-center gap-1 h-7 px-2 rounded-md text-[10.5px] font-semibold text-white/50 hover:text-white transition-colors ml-auto flex-shrink-0"
          >
      ...
      {/* Portal-rendered dropdowns — always on top of everything */}
      {typeof document !== 'undefined' && fromOpen && createPortal(
        <>
            ...
            <div className="p-1">
              {FROM_OPTIONS.map(o => {
                ...
                return (
                  ...
                  >
                    {o.icon ? (
                      <o.icon className={cn("w-3.5 h-3.5", o.color)} weight="fill" />
                    ) : (
                      <div className="w-3.5 h-3.5" />
                    )}
                    <span className="flex-1 text-[12px] font-semibold text-white">{o.label}</span>
      ...
      {typeof document !== 'undefined' && dateOpen && createPortal(
        <>
