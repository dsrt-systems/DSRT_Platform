...
const SOURCE_LABELS: Record<string, { label: string; color: string }> = {
  connect: { label: 'Connect', color: 'bg-white/[0.06] text-white/60 border-white/[0.1]' },
  application: { label: 'Application', color: 'bg-white/[0.06] text-white/60 border-white/[0.1]' },
  venture_invite: { label: 'Venture', color: 'bg-white/[0.06] text-white/60 border-white/[0.1]' },
  project_invite: { label: 'Project', color: 'bg-white/[0.06] text-white/60 border-white/[0.1]' },
  draft: { label: 'Draft', color: 'bg-white/[0.06] text-white/50 border-white/[0.1]' },
  scheduled: { label: 'Scheduled', color: 'bg-white/[0.06] text-white/50 border-white/[0.1]' },
}
...
export function ThreadRow({
  ...
  return (
    ...
    >
      {isSelected && <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-white" />}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <div className="flex items-center gap-1.5 min-w-0">
            {isUnread && !isDraftLike && <span className="w-1.5 h-1.5 rounded-full bg-white flex-shrink-0" />}
            <p className={cn('text-[13px] truncate', isUnread ? 'font-bold text-white' : 'font-semibold text-white/75')}>
              {sender?.display_name || 'Unknown'}
            </p>
            {thread.message_count > 1 && (
              <span className="text-[10.5px] text-white/35 flex-shrink-0 font-medium">{thread.message_count}</span>
            )}
          ...
          <span
            className={cn(
              'text-[10.5px] flex-shrink-0 transition-opacity',
        ...
        {(sourceMeta || thread.has_attachments || (isSnoozedFolder && snoozeUntil)) && (
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            {isSnoozedFolder && snoozeUntil && (
              <span className="text-[10px] font-semibold text-white/70 bg-white/[0.05] border border-white/[0.08] px-2 py-0.5 rounded inline-flex items-center gap-1">
                <Clock className="w-3 h-3" /> Wakes {formatWake(snoozeUntil)}
              </span>
            )}
            {sourceMeta && (
              <span className={cn('text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border', sourceMeta.color)}>
                {sourceMeta.label}
              </span>
            )}
      ...
      <div
        className={cn(
          ...
          'rounded-lg border border-white/[0.08] bg-[#0c0c12]/95 backdrop-blur-sm p-0.5 shadow-xl'
        )}
        onClick={(e) => e.stopPropagation()}
      >
        {isSnoozedFolder && (
          <>
            <ActionBtn title="Unsnooze — back to inbox" onClick={() => onUnsnooze?.(thread)}>
        ...
        {isScheduledFolder && (
          <>
            <ActionBtn title="Cancel schedule — open as draft" onClick={() => onUnschedule?.(thread)}>
              <ArrowClockwise className="w-4 h-4" />
            </ActionBtn>
            <ActionBtn title="Delete scheduled send" onClick={() => onDelete?.(thread)} danger>
              <Trash className="w-4 h-4" />
            </ActionBtn>
          </>
        )}
